...
def main():
    ...
    apps = {
        ...
        "Text Search / EBM - Sports Medicine": {"path": os.path.join(current_dir, "text_search_vector_db_sports", "app.py"), "dir": os.path.join(current_dir, "text_search_vector_db_sports"), "port": 8512}
    }
    ...
    if selected_app and selected_app != "Select":
        ...
        try:
            ...
            st.write(f"Running command: streamlit run {script_path} --server.port {port} --server.enableCORS false --server.enableXsrfProtection false")
            process = subprocess.Popen(['streamlit', 'run', script_path, '--server.port', str(port), '--server.enableCORS', 'false', '--server.enableXsrfProtection', 'false'], cwd=working_dir, shell=True)
